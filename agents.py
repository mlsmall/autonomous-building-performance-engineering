...
research_agent = create_react_agent(
    llm, tools=[tavily_tool], system_message="You are a highly trained researcher. You are tasked with \
...
input_validation_agent = create_react_agent(
    ...
    system_message=f"""You are an input validator. Use these exact validation rules from BuildingInput: {BuildingInput.model_json_schema()}
...
ashrae_lookup_agent = create_react_agent(
    ...
    system_message="You look up requested information from ASHRAE documents using RAG."
)

# recommendation_agent = create_react_agent(
#     llm,
#     tools=[recommendation_tool],
#     system_message="""You analyze building component performance and provide clear recommendations.
...
#     llm,
#     tools=[recommendation_tool],
#     system_message="Pass the input data directly to recommendation_tool without modifying it. \
...
recommendation_agent = create_react_agent(
    llm,
    tools=[recommendation_tool],
    system_message="Use recommendation_tool to get the differences. The tool returns a JSON string. Return this exact JSON string without any modification, parsing, or reformatting. Do not add any markdown formatting or explanation."
)
