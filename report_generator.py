...
def generate_performance_report(state):
    pdf = PDF()
    diff = {
            'total_heat_gain': state.get('proposed_total_heat_gain', 0) - state.get('baseline_total_heat_gain', 0),
    ...
    pdf.cell(60, 10, "Baseline Glass U-Value:", 0)
    pdf.cell(0, 10, f"{state.get('ashrae_glass_u', 0)}", ln=True)
    pdf.cell(60, 10, "Baseline Wall U-Value:", 0)
